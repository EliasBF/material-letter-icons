...
exports.generateIcons = function(input, cb) {
    ...
    baseSVG = fs.readFileSync(rootDir + config.src.svg[input.shape+'BasePath'], {encoding: 'utf8'});
    ...
    if (input.chars) {
        ...
        letters = input.pair ? [input.chars] : input.chars.split('');
    }
    ...
    async.eachSeries(letters, function(letter, cbAsync) {
        ...
        generateLetterIcon(letter, input, function(err) {
            // Handle errors
            if (err) {
                return cbAsync(err);
            }
...
function generateLetterIcon(letter, input, cb) {
    // Derive SVG from base letter SVG
    var letterSVG = baseSVG;
    
    // Get a random Material Design color
    var color = input.background || getRandomLetterColor();
    ...
    var outputSVGPath = path.join(input.output, config.dist.svg.outputPath, fileName + '.svg');
    var outputPNGPath = path.join(input.output, config.dist.png.outputPath, fileName + '.png');

    if (!fs.existsSync(path.dirname(outputSVGPath))) {
        fs.mkdirSync(path.dirname(outputSVGPath))
    }

    if (!fs.existsSync(path.dirname(outputPNGPath))) {
        fs.mkdirSync(path.dirname(outputPNGPath))
    }
...
function getIconFilename(letter) {
    // Not alphanumeric?
    if (!/^[0-9a-zA-Z]{1,2}$/.test(letter)) {
        // Return charcode instead
        if (letter.length > 1) {
            return 'ASCII-' + letter.split('').map(function(c) { return c.charCodeAt(0) }).join('_');
        }
